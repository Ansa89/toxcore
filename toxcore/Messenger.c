...
Messenger *new_messenger(Logger *log, Messenger_Options *options, unsigned int *error)
{
    ...
    if (options->udp_disabled) {
        ...
        m->net = new_networking(log, ip, options->port_range[0], options->port_range[1], options->traversal_type, &net_err);
    }
    ...
    if (options->tcp_server_port) {
        m->tcp_server = new_TCP_server(log, options->ipv6enabled, 1, &options->tcp_server_port, options->traversal_type,
